...
export const AuthProvider = ({ children }) => {
  ...
  // Disabled automatic status checking - account deactivation should only happen through admin action
  // useEffect(() => {
  ...
  const adminDeactivateUser = async (userId) => {
    try {
      const response = await apiService.updateUser(userId, { isActive: false });
      if (response.success) {
        toast({
  ...
  const adminReactivateUser = async (userId) => {
    try {
      const response = await apiService.updateUser(userId, { isActive: true });
      if (response.success) {
        toast({
